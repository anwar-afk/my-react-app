...
const ProgramForm = ({ programData, onSubmit, onCancel }) => {
  ...
  const [startDate, setStartDate] = useState(
    programData ? programData.startDate?.split("T")[0] : ""
  );
  const [endDate, setEndDate] = useState(
    programData ? programData.endDate?.split("T")[0] : ""
  );
  ...
  const [latitude, setLatitude] = useState(programData?.latitude || -6.2);
  const [longitude, setLongitude] = useState(
    programData?.longitude || 106.816666
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-50 pt-10">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 mb-10 overflow-y-auto max-h-[90vh]">
        <div className="sticky top-0 bg-white px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800">
            {programData
              ? `Edit Program: ${programData.title}`
              : "Buat Program Baru"}
          </h2>
          <button
            onClick={onCancel}
